...
test("integração visual da fase 4 é nativa, recortada e sem overlay de arte", () => {
  const stage4 = sources.get("stage4.js");
...
test("feedback visual de impacto da fase 4 permanece desacoplado do gameplay", () => {
  const parity = sources.get("stage4-impact-parity.js");
  assert.match(parity, /__shallStage4Debug/);
  assert.match(parity, /pointerEvents:\s*["']none["']/);
  assert.match(parity, /strokeText\(text/);
...
test("iluminação da fase 4 acompanha a câmera suavizada sem escrever no gameplay", () => {
  const parity = sources.get("stage4-lighting-parity.js");
  assert.match(parity, /__shallStage4Debug/);
  assert.match(parity, /visualCamera/);
  assert.match(parity, /dt\s*\*\s*4\.5/);
  assert.match(parity, /heroFocus\(debug, tick\)/);
  assert.match(parity, /prefers-reduced-motion/);
  assert.doesNotMatch(parity, /hero\.hp\s*=/);
  assert.doesNotMatch(parity, /boss\.hp\s*=/);
  assert.doesNotMatch(parity, /boss\.water\s*=/);
...
test("revelação do Água pOtávio replica staging das fases base sem tocar no gameplay", () => {
  const parity = sources.get("stage4-reveal-parity.js");
  const stage4Html = sources.get("stage4.html");
  assert.match(stage4Html, /id=["']stage4-boss-banner["']/);
  assert.match(stage4Html, /boss-banner\s+stage4-boss-banner/);
  assert.match(parity, /__shallStage4Debug/);
  assert.match(parity, /boss-reveal-active/);
  assert.match(parity, /prefers-reduced-motion/);
  assert.doesNotMatch(parity, /hero\.hp\s*=/);
